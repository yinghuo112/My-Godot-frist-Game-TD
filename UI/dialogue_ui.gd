...
func load_and_start_dialogue(file_path: String, start_id: String):
	...
	file.close()
	print("[DUI] 读取 ok, len=", content.length())
	var json = JSON.new()
...
func _show_dialogue(id: String):
	...
	print("[DUI]   打字: len=", current_text.length(), " speaker=", entry.get("speaker",""), " choices=", entry.has("choices"), " next=", entry.has("next_id"))
...
func _on_choice_at_index(index: int):
	...
	print("[DUI]   has_choices=", entry.has("choices"), " size=", entry.get("choices", []).size())
...
func _advance_text():
	var entry = dialogue_data.get(current_dialogue_id, {})
	print("[DUI] advance: stopped=", typing_timer.is_stopped(), " choices=", entry.has("choices"), " next=", entry.has("next_id"))

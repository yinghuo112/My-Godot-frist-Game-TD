...
# 完整伤害计算：命中 → 暴击 → 抗性
func _calculate_final_damage(target_node: Node2D) -> float:
	...
	return maxf(final_damage, 1.0)

# 命中目标：计算最终伤害并调用敌人的受伤函数
func _hit() -> void:
	if is_instance_valid(target) and target.has_method("take_damage"):
		var final_damage = _calculate_final_damage(target)
		if final_damage > 0:
...
func _on_area_entered(area: Area2D) -> void:
	if area.is_in_group("enemy"):
		...
		if enemy.has_method("take_damage"):
			var final_damage = _calculate_final_damage(enemy)
			if final_damage > 0:
